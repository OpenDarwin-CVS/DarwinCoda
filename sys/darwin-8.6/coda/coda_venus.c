...
#define CNV_VV2V_ATTR(top, fromp) \
	do { \
		...
		(top)->va_total_size = (fromp)->va_size; \
		(top)->va_iosize = (fromp)->va_blocksize; \
		(top)->va_access_time = (fromp)->va_atime; \
		(top)->va_modify_time = (fromp)->va_mtime; \
                (top)->va_change_time = (fromp)->va_ctime; \
		(top)->va_gen = (fromp)->va_gen; \
		(top)->va_flags = (fromp)->va_flags; \
		(top)->va_rdev = (fromp)->va_rdev; \
		(top)->va_data_alloc = (fromp)->va_bytes; \
...
#define CNV_V2VV_ATTR(top, fromp) \
	do { \
		...
		(top)->va_gid = (fromp)->va_gid; \
		(top)->va_fileid = (fromp)->va_fileid; \
		(top)->va_size = (fromp)->va_total_size; \
		(top)->va_blocksize = (fromp)->va_iosize; \
		(top)->va_atime = (fromp)->va_access_time; \
		(top)->va_mtime = (fromp)->va_modify_time; \
		(top)->va_ctime = (fromp)->va_change_time; \
		(top)->va_gen = (fromp)->va_gen; \
		(top)->va_flags = (fromp)->va_flags; \
		(top)->va_rdev = (fromp)->va_rdev; \
		(top)->va_bytes = (fromp)->va_data_alloc; \
		(top)->va_filerev = (fromp)->va_filerev; \
	} while (0)
	...
	struct ucred *cred, struct proc *p)
{
    ...
    error = copyin((user_addr_t)iap->vi.in, (void *)((char*)inp + (long)inp->data), 
		   iap->vi.in_size);
    if (error) {
