...
/* vfs kpi entries missing in vnode.h */
...
do {                                                                      \
    MALLOC(ptr, cast, size, M_TEMP, M_NOWAIT);            \
    if (ptr == 0) {                                                       \
        printf("kernel malloc returns 0 at %s:%d\n", __FILE__, __LINE__);  \
    }                                                                     \
} while (0)

#define CODA_FREE(ptr, size)  FREE(ptr, M_TEMP)
...
#define CODADEBUG(N, STMT)       { if (codadebug & CODADBGMSK(N)) {  } }
...
struct cnode {
    ...
    struct vnode_attr	 c_vattr; 	/* attributes */
 ...
 */
#define vftomi(mp)    ((struct coda_mntinfo *)vfs_fsprivate(mp))
#define	CODA_MOUNTED(vfsp)   (vftomi((vfsp)) != (struct coda_mntinfo *)0)

/*
 * vnode pointer to mount info
 */
#define vtomi(x) ((struct coda_mntinfo *)vnode_vfsfsprivate(x))
