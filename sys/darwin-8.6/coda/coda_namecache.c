    ...
    struct cnode *cp;
{
	...
	CODA_NC_HSHREM(cncp);
	vnode_rele(CTOV(cncp->dcp)); 
	vnode_rele(CTOV(cncp->cp));
	//crfree(cncp->cred);
    }
	...
	enum dc_status dcstat;
{
    int error=0;
    /* 
     * remove an entry -- vnode_rele(cncp->dcp, cp), crfree(cred),
    ...
    if ((dcstat == IS_DOWNCALL) && !vnode_isinuse(CTOV(cncp->dcp),1)) {
        cncp->dcp->c_flags |= C_PURGING;
    }
    vnode_rele(CTOV(cncp->dcp)); 
    
    if ((dcstat == IS_DOWNCALL) && !vnode_isinuse(CTOV(cncp->cp),1)) {
	...
	enum dc_status dcstat;
{
	...
	for (cncp = CODA_NC_LRUGET(coda_nc_lru);
		...
		if (CODA_NC_VALID(cncp)) {
			...
			}  It seems like pure text prototypes are not used in MacOS X, There's no KPI and VTEXT is never used in XNU. */

			if ((dcstat == IS_DOWNCALL) 
			    && !vnode_isinuse(CTOV(cncp->cp),1))
			{
				cncp->cp->c_flags |= C_PURGING;
			}
			vnode_rele(CTOV(cncp->cp));  
