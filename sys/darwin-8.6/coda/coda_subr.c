	...
	enum dc_status dcstat;
{
	...
	for (hash = 0; hash < CODA_CACHESIZE; hash++) {
		for (cp = coda_cache[hash]; cp != NULL; cp = CNODE_NEXT(cp)) {
			if (vnode_mount(CTOV(cp)) == whoIam) {
#ifdef	DEBUG
				myprintf(("coda_kill: vp %p, cp %p\n", CTOV(cp), cp));
#endif
				count++;
				CODADEBUG(CODA_FLUSH, 
					  myprintf(("Live cnode fid %s flags %d\n",
						    coda_f2s(&cp->c_fid),
						    cp->c_flags)));
			}
		}
...
coda_testflush(void)
{
	...
	for (cp = coda_cache[hash];
	     cp != NULL;
	     cp = CNODE_NEXT(cp)) {  
	    myprintf(("Live cnode fid %s count (inaccessible)\n",
		      coda_f2s(&cp->c_fid)));
	}
    }
}
    ...
    switch (opcode) {
	  ...
	  if (cp != NULL) {
	      ...
	      ASSERT_VOP_LOCKED(CTOV(cp), "coda HandleDownCall");
              /* Shouldn't happen in Darwin 
	      if (CTOV(cp)->v_vflag & VV_TEXT)
		  error = coda_vmflush(cp);
              */
	      CODADEBUG(CODA_ZAPFILE, 
			myprintf(("zapfile: fid = %s, refcnt = (inaccessible), error = %d\n",
				  coda_f2s(&cp->c_fid)/*, CTOV(cp)->v_usecount - 1*/, error)););
    if (!vnode_isinuse(CTOV(cp),1)) {
	  ...
	  if (cp != NULL) {
		  ...
		  "zapdir: fid = %s, refcnt = (inaccessible)\n",
	  ...
	  if (cp != NULL) {
	      ...
	      CODADEBUG(CODA_PURGEFID, myprintf((
			 "purgefid: fid = %s, refcnt = (inaccessible), error = %d\n",
	  ...
	  if (cp != NULL) { 
	      ...
	      CODADEBUG(CODA_REPLACE, myprintf((
			...
			coda_f2s(&cp->c_fid), cp));)	      vnode_rele(CTOV(cp));
	  }
	  return (0);
      }
      default:
